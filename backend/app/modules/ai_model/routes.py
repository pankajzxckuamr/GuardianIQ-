...
def create_ai_model(ai_model_in: AIModelCreate, db: Session = Depends(get_db)):
...
def update_ai_model(model_id: int, ai_model_in: AIModelUpdate, db: Session = Depends(get_db)):
    db_obj = db.query(AIModel).filter(AIModel.id == model_id).first()
    if not db_obj:
        raise HTTPException(status_code=404, detail="AI Model not found")
