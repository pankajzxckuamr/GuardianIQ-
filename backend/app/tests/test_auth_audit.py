...
class AuthAuditIntegrationTests(unittest.TestCase):
    ...
    def test_failed_login_records_audit_event(self):
        ...
        initial_count = self.db.query(AuditEvent).count()
        ...
        new_count = self.db.query(AuditEvent).count()
        self.assertEqual(new_count, initial_count + 1)

        # 4. Verify event details
        latest_event = self.db.query(AuditEvent).order_by(AuditEvent.created_at.desc()).first()
