...
import { fetchPolicies } from "../../services/policies/policyService";
...
export const AgentPoliciesTab: React.FC<AgentPoliciesTabProps> = ({ agentId }) => {
  ...
  const [policiesMap, setPoliciesMap] = useState<Record<string, Policy>>({});
  ...
  const loadPolicies = async () => {
    setLoading(true);
    try {
      const [res, polsRes] = await Promise.allSettled([
        serverClient.get<any>(
          `/api/v1/policy-bindings/effective?target_type=AGENT&target_id=${agentId}`
        ),
        fetchPolicies(),
      ]);

      if (polsRes.status === "fulfilled" && polsRes.value?.data) {
        const raw = polsRes.value.data as any;
        const items: Policy[] = Array.isArray(raw) ? raw : raw.items || [];
      ...
      {loading ? (
        ...
        <div className={styles.tableWrapper}>
          <table className={styles.table}>
            ...
            <tbody>
              {policies.map((p, idx) => {
                const matched = policiesMap[p.policy_id];
                const policyName = matched?.name || p.policy_name || p.policy_id.slice(0, 8) + "...";
                const policyCode = matched?.policy_code || p.policy_code || p.policy_id;
